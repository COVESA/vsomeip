...
udp_provider_impl::udp_provider_impl(
		factory *_factory,
		const endpoint *_endpoint,
		boost::asio::io_service &_is)
		: provider_base_impl(_factory, VSOMEIP_MAX_UDP_MESSAGE_SIZE, _is),
...
void udp_provider_impl::start() {
	receive();
}

void udp_provider_impl::restart() {
	receive();
}

void udp_provider_impl::connect() {
}

void udp_provider_impl::receive() {
...
void udp_provider_impl::stop() {
}

void udp_provider_impl::send_queued() {
	...
	socket_.async_send_to(
		...
		target,
		boost::bind(&provider_base_impl::sent, this,
					boost::asio::placeholders::error,
					boost::asio::placeholders::bytes_transferred));
}

ip_address udp_provider_impl::get_remote_address() const {
	return remote_endpoint_.address().to_string();
}

ip_port udp_provider_impl::get_remote_port() const {
	return remote_endpoint_.port();
}

ip_protocol udp_provider_impl::get_protocol() const {
	return ip_protocol::UDP;
}

ip_version udp_provider_impl::get_version() const {
	return (remote_endpoint_.protocol().v4() == ip::udp::v4() ?
			 ip_version::V4 : ip_version::V6);
}

const uint8_t * udp_provider_impl::get_received() const {
