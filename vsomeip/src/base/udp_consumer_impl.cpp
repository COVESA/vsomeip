...
udp_consumer_impl::udp_consumer_impl(
		factory *_factory,
		const endpoint *_endpoint,
		boost::asio::io_service &_is)
		: consumer_base_impl(_factory, VSOMEIP_MAX_UDP_MESSAGE_SIZE, _is),
		  socket_(is_),
...
void udp_consumer_impl::stop() {
	if (socket_.is_open())
		socket_.close();
}

void udp_consumer_impl::restart() {
	socket_.async_receive_from(
	        boost::asio::buffer(received_), remote_endpoint_,
	        boost::bind(&participant_impl::received, this,
	          boost::asio::placeholders::error,
...
void udp_consumer_impl::send_queued() {
	socket_.async_send(
...
std::string udp_consumer_impl::get_remote_address() const {
	return remote_endpoint_.address().to_string();
}

uint16_t udp_consumer_impl::get_remote_port() const {
	return remote_endpoint_.port();
}

ip_protocol udp_consumer_impl::get_protocol() const {
	return ip_protocol::UDP;
}

ip_version udp_consumer_impl::get_version() const {
	return (remote_endpoint_.protocol().v4() == ip::udp::v4() ?
			ip_version::V4 : ip_version::V6);
}

const uint8_t * udp_consumer_impl::get_received() const {
