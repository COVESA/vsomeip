...
consumer_base_impl::consumer_base_impl(
		factory *_factory,
...
bool consumer_base_impl::send(const message_base *_message,  bool _flush) {
...
bool consumer_base_impl::send(const uint8_t *_data, const uint32_t _size, bool _flush) {
	...
	if (_flush) {
		...
		flush_timer_.async_wait(boost::bind(&consumer_base_impl::flush, this,
...
void consumer_base_impl::flush(const boost::system::error_code &_error_code) {
	if (!_error_code) {
		(void)flush();
	}
}

bool consumer_base_impl::flush() {
...
// Private
//
void consumer_base_impl::connected(
