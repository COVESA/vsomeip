...
void send_tcp_messages(vsomeip::client *c, vsomeip::message *m, bool flush) {
...
void send_udp_messages(vsomeip::client *c, vsomeip::message *m, bool flush) {
	for (int i = 1; i <= ITERATIONS; i++) {
		udp_mutex.lock();
		c->send(m, flush);
		udp_mutex.unlock();
...
void tcp_func(void *id) {
	...
	while (FOREVER) {
		std::cout << "TCP Client " << id << ": run " << i++
				  ...
				  << std::endl;
		send_tcp_messages(tcp_client, tcp_test_message, true);
...
void udp_func(void *id) {
	...
	while (FOREVER) {
		std::cout << "UDP Client " << id << ": run " << i++
				  << " sending " << ITERATIONS << " messages."
				  << std::endl;
		send_udp_messages(udp_client, udp_test_message, true);
		boost::this_thread::sleep(boost::posix_time::seconds(TIMEOUT_BETWEEN));
	}
}
#endif
