...
public:
    ...
    bool init() {
        ...
        std::cout << "Client settings [protocol=" << (use_tcp_ ? "TCP" : "UDP") << ":quiet=" << (be_quiet_ ? "true" : "false")
                  << ":cycle=" << cycle_ << "]" << std::endl;

        app_->register_state_handler(std::bind(&client_sample::on_state, this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, SAMPLE_INSTANCE_ID + 1, vsomeip::ANY_METHOD,
                                       std::bind(&client_sample::on_message, this, std::placeholders::_1));

        request_->set_service(SAMPLE_SERVICE_ID + 1);
        request_->set_instance(SAMPLE_INSTANCE_ID + 1);
 ...
 */
        app_->register_availability_handler(
                SAMPLE_SERVICE_ID + 1, SAMPLE_INSTANCE_ID + 1,
                std::bind(&client_sample::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        return true;
    }
    ...
    void stop() {
        ...
        app_->release_service(SAMPLE_SERVICE_ID + 1, SAMPLE_INSTANCE_ID + 1);
    ...
    void on_state(vsomeip::state_type_e _state) {
        if (_state == vsomeip::state_type_e::ST_REGISTERED) {
            app_->request_service(SAMPLE_SERVICE_ID + 1, SAMPLE_INSTANCE_ID + 1);
        }
    }
    ...
    void on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
        ...
        if ((SAMPLE_SERVICE_ID + 1 )== _service && (SAMPLE_INSTANCE_ID + 1 ) == _instance) {
