...
# --- CONFIGURATION ---
NS_NAME="service-ns"           # Name of the namespace (change as needed)
PARENT_IF="enp0s3"             # Main VM interface (change as needed)
VLAN_ID="20"                   # VLAN ID (change as needed)
...
GW_IP="192.168.20.1"           # Gateway (Router VM's VLAN IP)
SRV_IP="192.168.20.2"
CL_IP="192.168.10.2"
...
sudo ip netns exec "$NS_NAME" ip addr add "$NS_SRV" dev "$VLAN_IF" || { echo "Failed to assign IP to $VLAN_IF"; exit 1; }
...
sudo ip netns exec $NS_NAME ip xfrm policy add src $NS_SRV dst $NS_CL dir out \
  tmpl src $SRV_IP dst $CL_IP proto esp mode tunnel 2>/dev/null || true
sudo ip netns exec $NS_NAME ip xfrm policy add src $NS_CL dst $NS_SRV dir in \
...
echo "Pinging client $CL_IP from $NS_NAME:"
sudo ip netns exec "$NS_NAME" ping -c 4 $CL_IP || true

#sudo ip netns exec service-ns ping -c 4 192.168.20.1
#sudo ip netns exec service-ns ping -c 4 192.168.10.1
#sudo ip netns exec service-ns ping -c 4 192.168.10.2
