...
sudo ip netns exec "$NS_NAME" ip addr add "$NS_SRV" dev "$VLAN_IF"
...
sudo ip netns exec $NS_NAME ip xfrm state add src $SRV_IP dst $CL_IP proto esp spi 0x100 mode transport \
  enc aes 0x1234567890abcdef1234567890abcdef auth sha1 0xabcdef1234567890abcdef1234567890abcdef12 2>/dev/null || true
sudo ip netns exec $NS_NAME ip xfrm state add src $CL_IP dst $SRV_IP proto esp spi 0x101 mode transport \
...
sudo ip netns exec "$NS_NAME" ping -c 4 $CL_GW_IP || true
