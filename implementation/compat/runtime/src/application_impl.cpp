...
void application_impl::offer_event(service_t _service, instance_t _instance, event_t _event, const std::set<eventgroup_t>& _eventgroups,
                                   bool _is_field) {
    ...
    if (_is_field)
        ...
        const bool is_selective = is_selective_event(_service, _instance, _eventgroups);
...
void application_impl::request_event(service_t _service, instance_t _instance, event_t _event, const std::set<eventgroup_t>& _eventgroups,
    ...
    const vsomeip_v3::event_type_e its_type = (_is_field) ? vsomeip_v3::event_type_e::ET_FIELD : vsomeip_v3::event_type_e::ET_EVENT;
...
void application_impl::send(std::shared_ptr<message> _message, bool _flush = true) {
...
void application_impl::notify(service_t _service, instance_t _instance, event_t _event, std::shared_ptr<payload> _payload) const {
...
void application_impl::register_state_handler(state_handler_t _handler) {
    ...
    impl_->register_state_handler([_handler](vsomeip_v3::state_type_e _state) { _handler(static_cast<vsomeip::state_type_e>(_state)); });
}
...
void application_impl::register_message_handler(service_t _service, instance_t _instance, method_t _method, message_handler_t _handler) {

    impl_->register_message_handler(_service, _instance, _method, [_handler](const std::shared_ptr<vsomeip_v3::message>& _message) {
...
void application_impl::register_availability_handler(service_t _service, instance_t _instance, availability_handler_t _handler,
                                                     major_version_t _major, minor_version_t _minor) {

    impl_->register_availability_handler(_service, _instance, _handler, _major, _minor);
}
...
void application_impl::register_subscription_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    ...
    impl_->register_subscription_handler(_service, _instance, _eventgroup,
                                         [_handler](client_t _client, vsomeip::uid_t _uid, vsomeip::gid_t _gid, bool _accepted) {
                                             (void)_uid;
                                             (void)_gid;
...
void application_impl::register_subscription_error_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    ...
    impl_->register_subscription_status_handler(
            ...
            [_handler](service_t _service, instance_t _instance, eventgroup_t _eventgroup, event_t _event, uint16_t _error) {
                (void)_service;
                (void)_instance;
                (void)_eventgroup;
                (void)_event;
...
void application_impl::unregister_subscription_error_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup) {

    impl_->unregister_subscription_status_handler(_service, _instance, _eventgroup, ERROR_HANDLER_DUMMY_EVENT);
}
...
void application_impl::offer_event(service_t _service, instance_t _instance, event_t _event, const std::set<eventgroup_t>& _eventgroups,
    ...
    impl_->offer_event(
            _service, _instance, _event, _eventgroups, its_type, _cycle, _change_resets_cycle, true,
            [_epsilon_change_func](const std::shared_ptr<vsomeip_v3::payload>& _lhs, const std::shared_ptr<vsomeip_v3::payload>& _rhs) {
...
void application_impl::notify(service_t _service, instance_t _instance, event_t _event, std::shared_ptr<payload> _payload,
    ...
    if (_payload) {
        auto its_payload = std::dynamic_pointer_cast<payload_impl>(_payload);
        impl_->notify(_service, _instance, _event, its_payload->get_impl(), _force);
    } else {
...
void application_impl::notify_one(service_t _service, instance_t _instance, event_t _event, std::shared_ptr<payload> _payload,
    ...
    if (_payload) {
        auto its_payload = std::dynamic_pointer_cast<payload_impl>(_payload);
        impl_->notify_one(_service, _instance, _event, its_payload->get_impl(), _client, _force);
    } else {
        impl_->notify_one(_service, _instance, _event, nullptr, _client, _force);
    }
}
...
bool application_impl::are_available(available_t& _available, service_t _service, instance_t _instance, major_version_t _major,
    ...
    return impl_->are_available(_available, _service, _instance, _major, _minor);
}
...
void application_impl::register_subscription_status_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    ...
    register_subscription_status_handler(_service, _instance, _eventgroup, _event, _handler, false);
}
...
void application_impl::register_subscription_status_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    ...
    if (_is_selective) {
        ...
        // here.
        impl_->request_event(_service, _instance, _event, its_eventgroups, vsomeip_v3::event_type_e::ET_SELECTIVE_EVENT);
    }

    impl_->register_subscription_status_handler(
            _service, _instance, _eventgroup, _event,
            [_handler](const vsomeip_v3::service_t _service, const vsomeip_v3::instance_t _instance,
                       const vsomeip_v3::eventgroup_t _eventgroup, const vsomeip_v3::event_t _event, const uint16_t _error) {
                if (_handler)
...
void application_impl::get_offered_services_async(offer_type_e _offer_type, offered_services_handler_t _handler) {
    ...
    impl_->get_offered_services_async(static_cast<vsomeip_v3::offer_type_e>(_offer_type), _handler);
}
...
void application_impl::register_async_subscription_handler(service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    ...
    impl_->register_async_subscription_handler(_service, _instance, _eventgroup,
                                               [_handler](client_t _client, vsomeip::uid_t _uid, vsomeip::gid_t _gid, bool _accepted,
...
////////////////////////////////////////////////////////////////////////////////
void application_impl::set_offer_acceptance_required(ip_address_t _address, const std::string _path, bool _enable) {
...
vsomeip::application::offer_acceptance_map_type_t application_impl::get_offer_acceptance_required() {

    VSOMEIP_ERROR << __func__ << ": Must not be called from compatibility layer.";
    return vsomeip::application::offer_acceptance_map_type_t();
}

void application_impl::register_offer_acceptance_handler(offer_acceptance_handler_t _handler) {

    (void)_handler;
    VSOMEIP_ERROR << __func__ << ": Must not be called from compatibility layer.";
}

void application_impl::register_reboot_notification_handler(reboot_notification_handler_t _handler) {

    (void)_handler;
    VSOMEIP_ERROR << __func__ << ": Must not be called from compatibility layer.";
}

void application_impl::register_routing_ready_handler(routing_ready_handler_t _handler) {

    (void)_handler;
    VSOMEIP_ERROR << __func__ << ": Must not be called from compatibility layer.";
}

void application_impl::register_routing_state_handler(routing_state_handler_t _handler) {
...
void application_impl::remove_security_policy_configuration(uint32_t _uid, uint32_t _gid, security_update_handler_t _handler) {
...
bool application_impl::is_selective_event(vsomeip::service_t _service, vsomeip::instance_t _instance,
