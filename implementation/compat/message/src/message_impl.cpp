...
message_impl::message_impl(const std::shared_ptr<vsomeip_v3::message>& _impl) : message_base_impl(_impl) { }
...
bool message_impl::serialize(serializer* _to) const {
    ...
    (void)_to;
    VSOMEIP_ERROR << "message_impl::" << __func__ << ": Must not be called from compatibility layer.";
    return false;
}
