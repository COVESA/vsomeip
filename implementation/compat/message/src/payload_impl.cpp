...
payload_impl::payload_impl(const std::shared_ptr<vsomeip_v3::payload>& _impl) : impl_(_impl) { }
...
bool payload_impl::serialize(serializer* _to) const {
    ...
    (void)_to;
    VSOMEIP_ERROR << "payload_impl::" << __func__ << ": Must not be called from compatibility layer.";
    return false;
}
