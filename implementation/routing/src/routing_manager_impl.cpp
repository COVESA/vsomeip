...
std::tuple<std::shared_ptr<vsomeip::e2e::profile_interface::checker>,
           std::shared_ptr<vsomeip::e2e::profile_interface::protector>>
process_e2e_config(std::shared_ptr<vsomeip::cfg::e2e> config) {
    if(config->profile == "CRC8") {
        return process_e2e_profile<vsomeip::e2e::profile01::profile_config,
                                   vsomeip::e2e::profile01::profile_01_checker,
                                   vsomeip::e2e::profile01::protector>(config);
    }

    if(config->profile == "CRC32") {
        return process_e2e_profile<vsomeip::e2e::profile_custom::profile_config,
                                   vsomeip::e2e::profile_custom::profile_custom_checker,
                                   vsomeip::e2e::profile_custom::protector>(config);
    }
...
void routing_manager_impl::init() {
    ...
    if( configuration_->is_e2e_enabled()) {
        ...
        for (auto &identifier : its_e2e_configuration) {
            ...
            if(is_e2e_profile_supported(its_cfg->profile)) {
                ...
                std::tie(checker, protector) = process_e2e_config(its_cfg);
