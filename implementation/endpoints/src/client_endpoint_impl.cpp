...
template<typename Protocol>
void client_endpoint_impl<Protocol>::flush_queue() {

    std::unique_lock its_lock(mutex_);

    // STEP 1: Force current train to dispatched_trains_ if not empty
    if (!train_->buffer_->empty()) {
        schedule_train();
        train_ = std::make_shared<train>();
        train_->departure_ = std::chrono::steady_clock::now();
    }

    // STEP 2: Move ALL dispatched_trains_ to queue at once
    while (!dispatched_trains_.empty()) {
        auto its_dispatched = dispatched_trains_.begin();
        for (const auto& train_item : its_dispatched->second) {
            queue_size_ += train_item->buffer_->size();
            queue_.emplace_back(train_item->buffer_, 0);
        }
        dispatched_trains_.erase(its_dispatched);
    }

    // STEP 3: Force start sending if idle
    ...
    // STEP 4: Wait for everything to be sent
    boost::system::error_code its_error;
    std::uint32_t retry_count(0);
    while (true) {
        size_t send_buffer_size = get_send_buffer_size(its_error);
        if (its_error) {
            VSOMEIP_WARNING << "cei::" << __func__ << ": fail to read send_buffer_size  "
                            << "(" << its_error.value() << "): " << its_error.message() << ", " << get_remote_information();
            break;
        }
        std::shared_ptr<train> its_train(train_);
        if (queue_.size() > 0 || is_sending_ || send_buffer_size > 0) {
            VSOMEIP_INFO << "cei::" << __func__ << ": waiting[" << retry_count
                         << "] on close to send remaining data, remote: " << get_remote_information() << ", socket state > "
                         << to_string(state_.load()) << ", is_sending_ > " << is_sending_ << ", queue_.size() > " << queue_.size()
        ...
        if (retry_count > VSOMEIP_TCP_CLOSE_SEND_BUFFER_RETRIES) {
            VSOMEIP_WARNING << "cei::" << __func__ << ": max retries reached to send! will drop remaining data on close "
                            << ", remote: " << get_remote_information() << ", socket state > " << to_string(state_.load());
            break;
        }
    }
}
...
template<typename Protocol>
size_t client_endpoint_impl<Protocol>::get_send_buffer_size(boost::system::error_code& _ec) {
#if defined(__linux__) || defined(__QNX__)
    if constexpr (std::is_same_v<Protocol, boost::asio::ip::tcp>) {
        io_control_operation<std::size_t> send_buffer_size_cmd(TIOCOUTQ);
        {
            ...
            if (socket_ && socket_->is_open()) {
                ...
                VSOMEIP_WARNING << "cei::" << __func__ << ": socket is not open, " << get_remote_information();
        ...
        if (_ec) {
            VSOMEIP_WARNING << "cei::" << __func__ << ": fail to read send_buffer_size  "
                            << "(" << _ec.value() << "): " << _ec.message() << ", " << get_remote_information();
            return 0;
        }
        return send_buffer_size_cmd.get();
    } else {
        _ec.clear();
        return 0; // not supported for UDP
    }
#else
    _ec.clear();
