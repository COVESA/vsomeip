...
std::shared_ptr<abstract_netlink_connector> asio_socket_factory::create_netlink_connector(
        boost::asio::io_context& _io, const boost::asio::ip::address& _address,
        const boost::asio::ip::address& _multicast_address, bool _is_requiring_link) {
    return std::make_shared<netlink_connector>(_io, _address, _multicast_address,
                                               _is_requiring_link);
}
#endif
...
std::unique_ptr<tcp_acceptor>
asio_socket_factory::create_tcp_acceptor(boost::asio::io_context& _io) {
