...
udp_server_endpoint_impl::udp_server_endpoint_impl(
    ...
    unicast_recv_buffer_(VSOMEIP_MAX_UDP_MESSAGE_SIZE, 0), lifecycle_idx_(0),
    ...
    VSOMEIP_INFO << instance_name_ << "constructor";
}

udp_server_endpoint_impl::~udp_server_endpoint_impl() {
    VSOMEIP_INFO << instance_name_ << "destructor, lifecycle_idx=" << lifecycle_idx_.load();
}
...
void udp_server_endpoint_impl::init(const endpoint_type& _local,
    ...
    VSOMEIP_INFO << instance_name_ << "init: " << _local.address() << ":" << _local.port()
                 << ", lifecycle_idx=" << lifecycle_idx_.load();
    std::scoped_lock its_lock(sync_);
    init_unlocked(_local, _error);
    VSOMEIP_INFO << instance_name_ << "init: lifecycle_idx=" << lifecycle_idx_.load() << ", "
                 << _error.message();
}

void udp_server_endpoint_impl::init_unlocked(const endpoint_type& _local,
                                             boost::system::error_code& _error) {
    // The caller must hold the lock

    if (unicast_socket_) {
        if (local_port_ == _local.port() && local_.address() == _local.address()) {
            VSOMEIP_WARNING << instance_name_
                            << "init_unlocked: already initialized, lifecycle_idx="
                            << lifecycle_idx_.load();
            return;
        }

        VSOMEIP_WARNING << instance_name_ << "init_unlocked: reset unicast socket, lifecycle_idx="
                        << lifecycle_idx_.load();
    ...
    if (!unicast_socket_->is_open()) {
        ...
        if (_error) {
            VSOMEIP_ERROR << instance_name_ << "init_unlocked: failed to open socket, "
                          << _error.message();
            unicast_socket_.reset();
            return;
        }
    ...
    if (_error) {
        VSOMEIP_ERROR << instance_name_ << "init_unlocked: failed to reuse address, "
                      << _error.message();
        unicast_socket_.reset();
        return;
    }
    ...
    if (!its_device.empty()) {
        if (setsockopt(unicast_socket_->native_handle(), SOL_SOCKET, SO_BINDTODEVICE,
            ...
            == -1) {
            VSOMEIP_ERROR << instance_name_ << "init_unlocked: failed to bind to device, "
                          << _error.message();
            // Non-fatal error
            _error.clear();
        }
    ...
    if (_error) {
        VSOMEIP_ERROR << instance_name_ << "init_unlocked: failed to bind, " << _error.message();
        unicast_socket_.reset();
        return;
    }
    ...
    if (_local.address().is_v4()) {
        ...
        if (_error) {
            VSOMEIP_ERROR << instance_name_
                          << "init_unlocked: failed to configure IPv4 outbound interface, "
                          << _error.message();
            unicast_socket_.reset();
            return;
        }
        ...
        if (_error) {
            VSOMEIP_ERROR << instance_name_
                          << "init_unlocked: failed to configure IPv6 outbound interface, "
                          << _error.message();
            unicast_socket_.reset();
            return;
        }
    ...
    std::ignore = unicast_socket_->set_option(option, _error);
    if (_error) {
        VSOMEIP_ERROR << instance_name_ << "init_unlocked: failed to configure broadcast option, "
                      << _error.message();
        unicast_socket_.reset();
        return;
    }
    ...
    std::ignore = unicast_socket_->set_option(boost::asio::socket_base::receive_buffer_size(
                                                      static_cast<int>(its_udp_recv_buffer_size)),
                                              _error);

    if (_error) {
        VSOMEIP_ERROR << instance_name_
                      << "init_unlocked: failed to configure receive buffer size, "
                      << _error.message();
        // Non-fatal error
        _error.clear();
    }
    ...
    if (its_option.value() < 0 || its_option.value() < its_udp_recv_buffer_size) {
        ...
        if (_error) {
            VSOMEIP_INFO << instance_name_
                         << "init_unlocked: failed to force receive buffer size, "
                         << _error.message();
            // Non-fatal error
            _error.clear();
        }
    ...
    if (local_ != _local || local_port_ != _local.port()) {
        ...
        queue_limit_ = configuration_->get_endpoint_queue_limit(
...
void udp_server_endpoint_impl::start() {
    VSOMEIP_INFO << instance_name_ << "start, lifecycle_idx=" << lifecycle_idx_.load();
    std::scoped_lock its_lock(sync_);
    start_unlocked();
    VSOMEIP_INFO << instance_name_ << "start: done, lifecycle_idx=" << lifecycle_idx_.load();
}

void udp_server_endpoint_impl::stop() {
    VSOMEIP_INFO << instance_name_ << "stop, lifecycle_idx=" << lifecycle_idx_.load();
    std::scoped_lock its_lock(sync_);
    stop_unlocked();
    VSOMEIP_INFO << instance_name_ << "stop: done, lifecycle_idx=" << lifecycle_idx_.load();
}

void udp_server_endpoint_impl::restart(bool _force) {
    std::ignore = _force;

    VSOMEIP_INFO << instance_name_ << "restart, lifecycle_idx=" << lifecycle_idx_.load();
    std::scoped_lock its_lock(sync_);
    ...
    if (!its_error) {
        ...
        VSOMEIP_ERROR << instance_name_ << "restart: init failure " << its_error.message();
    }

    VSOMEIP_INFO << instance_name_ << "restart done, lifecycle_idx=" << lifecycle_idx_.load();
}
...
void udp_server_endpoint_impl::start_unlocked() {
    ...
    if (!is_stopped_) {
        VSOMEIP_INFO << instance_name_ << "start_unlocked: already started";
        return;
    }
    ...
    if (!unicast_socket_ || !unicast_socket_->is_open()) {
        VSOMEIP_ERROR << instance_name_ << "start_unlocked: init not called or not successful";
        return;
    }

    is_stopped_ = false;

    VSOMEIP_INFO << instance_name_ << "start_unlocked: start unicast data handler, lifecycle_idx="
                 << lifecycle_idx_.load();
...
void udp_server_endpoint_impl::stop_unlocked() {
    ...
    if (is_stopped_) {
        VSOMEIP_INFO << instance_name_ << "stop_unlocked: already stopped";
        return;
    }
        ...
        unicast_socket_->async_receive_from(
                ...
                [self = shared_ptr(), lifecycle_idx = lifecycle_idx_.load()](
                    ...
                    if (lifecycle_idx == self->lifecycle_idx_.load()
                        ...
                        VSOMEIP_WARNING
                                << self->instance_name_
                                << "receive_unicast_unlocked: stop data handler, lifecycle_idx="
                                << lifecycle_idx << " vs " << self->lifecycle_idx_.load() << ", "
                                << _error.message() << ", stopped=" << self->is_stopped_;
                    }
                });
    } else {
        VSOMEIP_WARNING << instance_name_
                        << "receive_unicast_unlocked: stop data handler, stopped=" << is_stopped_
                        << ", lifecycle_idx=" << lifecycle_idx_.load();
    }
}
...
//
void udp_server_endpoint_impl::receive_multicast_unlocked() {
    // The caller must hold the lock
    ...
    if (multicast_socket_ && multicast_socket_->is_open()) {
        auto its_storage = std::make_shared<udp_endpoint_receive_op::storage>(
                ...
                std::bind(&udp_server_endpoint_impl::on_multicast_received,
                          ...
                          std::placeholders::_4),
                &multicast_recv_buffer_[0], max_message_size_, is_v4_, boost::asio::ip::address(),
                std::numeric_limits<std::size_t>::min());
        multicast_socket_->async_wait(
                socket_type::wait_read,
                [self = shared_ptr(), its_storage,
                 lifecycle_idx = lifecycle_idx_.load()](boost::system::error_code const& _error) {
                    if (lifecycle_idx == self->lifecycle_idx_.load()
                        && _error != boost::asio::error::eof
                        && _error != boost::asio::error::connection_reset
                        && _error != boost::asio::error::operation_aborted) {
                        ...
                        VSOMEIP_WARNING
                                << self->instance_name_
                                << "receive_multicast_unlocked: stop data handler, lifecycle_idx="
                                << lifecycle_idx << " vs " << self->lifecycle_idx_.load() << ", "
                                << _error.message() << ", stopped=" << self->is_stopped_;
                    }
                });
        ...
        VSOMEIP_WARNING << instance_name_
                        << "receive_multicast_unlocked: stop data handler, stopped=" << is_stopped_
                        << ", lifecycle_idx=" << lifecycle_idx_.load();
    }
}
...
bool udp_server_endpoint_impl::send_queued(const target_data_iterator_type _it) {
    ...
    if (unicast_socket_) {
        ...
        VSOMEIP_WARNING << instance_name_ << "send_queued skipped, no socket!";
    }
    return result;
}
...
bool udp_server_endpoint_impl::send_queued_unlocked(const target_data_iterator_type _it) {
    ...
    if (auto its_me {std::dynamic_pointer_cast<udp_server_endpoint_impl>(shared_from_this())}) {
        ...
        unicast_socket_->async_send_to(
                ...
                [its_me, _it, its_entry](boost::system::error_code const& _error,
                                         std::size_t _bytes) {
                    if (!_error && its_me->on_unicast_sent_
                        && !_it->first.address().is_multicast()) {
                        its_me->on_unicast_sent_(&(its_entry.first)->at(0),
                                                 static_cast<uint32_t>(_bytes),
                                                 _it->first.address());
                    }
...
bool udp_server_endpoint_impl::is_joined_unlocked(const std::string& _address,
                                                  bool& _received) const {
...
void udp_server_endpoint_impl::join(const std::string& _address) {
    VSOMEIP_INFO << instance_name_ << "join: " << _address;
    std::scoped_lock its_lock(sync_);
...
void udp_server_endpoint_impl::join_unlocked(const std::string& _address) {
    ...
    try {
        ...
        VSOMEIP_ERROR << instance_name_ << "join_unlocked: exception " << e.what();
    }
}

void udp_server_endpoint_impl::leave(const std::string& _address) {
    VSOMEIP_INFO << instance_name_ << "leave: " << _address;
    std::scoped_lock its_lock(sync_);
...
void udp_server_endpoint_impl::leave_unlocked(const std::string& _address) {
    ...
    try {
        ...
        VSOMEIP_ERROR << instance_name_ << "leave_unlocked: exception " << e.what();
    }
}
...
void udp_server_endpoint_impl::on_unicast_received(boost::system::error_code const& _error,
    ...
    if (_error) {
        VSOMEIP_ERROR << instance_name_ << "on_unicast_received: " << _error.message();
    } else {
        on_message_received_unlocked(_error, _bytes, false, unicast_remote_, unicast_recv_buffer_);
    }
}
...
void udp_server_endpoint_impl::on_multicast_received(
    ...
    if (_error) {
        VSOMEIP_ERROR << instance_name_ << "on_multicast_received: " << _error.message();
    } else {
        ...
        if (!own_message) {
            if (own_subnet) {
                on_message_received_unlocked(_error, _bytes, true, _sender, multicast_recv_buffer_);
            }
        } else if (own_callback) {
            own_callback(&multicast_recv_buffer_[0], static_cast<uint32_t>(_bytes),
...
void udp_server_endpoint_impl::on_message_received_unlocked(boost::system::error_code const& _error,
        ...
        if (!_error && 0 < _bytes) {
            ...
            do {
                ...
                if (read_message_size > MESSAGE_SIZE_UNLIMITED) {
                    VSOMEIP_ERROR << instance_name_
                                  << "on_message_received_unlocked: message size exceeds allowed "
                                     "maximum!";
                    return;
                }
                ...
                if (current_message_size > VSOMEIP_SOMEIP_HEADER_SIZE
                    ...
                    if (remaining_bytes - current_message_size > remaining_bytes) {
                        VSOMEIP_ERROR << instance_name_
                                      << "on_message_received_unlocked: buffer underflow!";
                        return;
                    }
                    ...
                    if (current_message_size > VSOMEIP_RETURN_CODE_POS
                        && (_buffer[i + VSOMEIP_PROTOCOL_VERSION_POS] != VSOMEIP_PROTOCOL_VERSION
                            || !utility::is_valid_message_type(
                                    tp::tp::tp_flag_unset(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS]))
                        ...
                        if (_buffer[i + VSOMEIP_PROTOCOL_VERSION_POS] != VSOMEIP_PROTOCOL_VERSION) {
                            VSOMEIP_ERROR
                                    << instance_name_
                                    << "on_message_received_unlocked: wrong protocol version: 0x"
                                    << std::hex << std::setfill('0') << std::setw(2)
                                    << static_cast<uint32_t>(
                                               _buffer[i + VSOMEIP_PROTOCOL_VERSION_POS])
                                    << " local: " << get_address_port_local_unlocked()
                            ...
                            VSOMEIP_ERROR
                                    << instance_name_
                                    << "on_message_received_unlocked: invalid message type: 0x"
                                    << std::hex << std::setfill('0') << std::setw(2)
                                    << static_cast<uint32_t>(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS])
                                    << " local: " << get_address_port_local_unlocked()
                                    << " remote: " << its_remote_address << ":" << std::dec
                                    << its_remote_port;
                            ...
                            VSOMEIP_ERROR
                                    << instance_name_
                                    << "on_message_received_unlocked: invalid return code: 0x"
                                    << std::hex << std::setfill('0') << std::setw(2)
                                    << static_cast<uint32_t>(_buffer[i + VSOMEIP_RETURN_CODE_POS])
                                    << " local: " << get_address_port_local_unlocked()
                                    << " remote: " << its_remote_address << ":" << std::dec
                                    << its_remote_port;
                                   ...
                                   && get_local_port() == configuration_->get_sd_port()) {
                            VSOMEIP_WARNING << instance_name_
                                            << "on_message_received_unlocked: not a SD message,"
                                            << " local: " << get_address_port_local_unlocked()
                                            << " remote: " << its_remote_address << ":" << std::dec
                                            << its_remote_port;
                        } else {
                    ...
                    if (tp::tp::tp_flag_is_set(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS])) {
                        ...
                        if (its_instance != ANY_INSTANCE) {
                            if (!tp_segmentation_enabled(its_service, its_instance, its_method)) {
                                VSOMEIP_WARNING << instance_name_
                                                << "on_message_received_unlocked: SomeIP/TP "
                                                   "message for service: 0x"
                                                << std::hex << its_service << " method: 0x"
                                                << its_method << " which is not configured for TP:"
                                                << " local: " << get_address_port_local_unlocked()
                                                << " remote: " << its_remote_address << ":"
                        ...
                        if (res.first) {
                            ...
                            its_host->on_message(&res.second[0],
                                                 static_cast<uint32_t>(res.second.size()), this,
                                                 _is_multicast, VSOMEIP_ROUTING_CLIENT, nullptr,
                                                 its_remote_address, its_remote_port);
                        }
                    } else {
                        if (its_service != VSOMEIP_SD_SERVICE
                            ...
                            VSOMEIP_ERROR << instance_name_
                                          << "on_message_received_unlocked: unreliable vSomeIP SD "
                                             "message with too short length field"
                                          << " local: " << get_address_port_local_unlocked()
                                          << " remote: " << its_remote_address << ":" << std::dec
                                          << its_remote_port;
                        }
                    ...
                    VSOMEIP_ERROR << instance_name_
                                  << "on_message_received_unlocked: unreliable vSomeIP message "
                                     "with bad length field local: "
                                  << get_address_port_local_unlocked()
                                  << " remote: " << its_remote_address << ":" << std::dec
                                  << its_remote_port;
                    if (remaining_bytes > VSOMEIP_SERVICE_POS_MAX) {
                        ...
                        if (its_service != VSOMEIP_SD_SERVICE) {
                            if (read_message_size == 0) {
                                VSOMEIP_ERROR << instance_name_
                                              << "on_message_received_unlocked: unreliable vSomeIP "
                                                 "message with SomeIP message length 0!";
                            } else {
                                auto its_endpoint_host = endpoint_host_.lock();
                                if (its_endpoint_host) {
                                    its_endpoint_host->on_error(
                                            &_buffer[i], static_cast<uint32_t>(remaining_bytes),
...
bool udp_server_endpoint_impl::is_same_subnet_unlocked(
...
void udp_server_endpoint_impl::print_status() {
    ...
    VSOMEIP_ERROR << instance_name_ << "status use: " << std::dec << local_port_
                  << " number targets: " << std::dec << targets_.size()
                  << " recv_buffer: " << std::dec << unicast_recv_buffer_.capacity()
                  << " multicast_recv_buffer: " << std::dec << multicast_recv_buffer_.capacity();
    ...
    for (const auto& c : targets_) {
        ...
        VSOMEIP_INFO << instance_name_ << "status use: client: " << c.first.address().to_string()
                     << ":" << std::dec << c.first.port() << " queue: " << std::dec
                     << its_queue_size << " data: " << std::dec << its_data_size;
    }
}
...
std::string udp_server_endpoint_impl::get_address_port_local_unlocked() const {
    // The caller doesn't have to hold the lock

    std::shared_ptr<socket_type> unicast_socket = unicast_socket_;
...
void udp_server_endpoint_impl::set_multicast_option(const boost::asio::ip::address& _address,
                                                    bool _is_join,
                                                    boost::system::error_code& _error) {
    VSOMEIP_INFO << instance_name_ << "set_multicast_option: " << (_is_join ? "join " : "leave ")
                 << _address << ", lifecycle_idx=" << lifecycle_idx_.load()
                 << ", stopped=" << is_stopped_;
    ...
    if (is_stopped_) {
        VSOMEIP_INFO << instance_name_
                     << "set_multicast_option: ignored because server is stopping";
        return;
    }

    bool has_joined =
            multicast_socket_ && join_status_.find(_address.to_string()) != join_status_.end();
    ...
    if (_is_join) {
        ...
        if (!multicast_socket_) {
            ...
            if (!multicast_socket_->is_open()) {
                ...
                if (_error) {
                    VSOMEIP_ERROR << instance_name_
                                  << "set_multicast_option: failed to open socket, "
                                  << _error.message();
                    multicast_socket_.reset();
                    return;
                }
            ...
            if (_error) {
                VSOMEIP_ERROR << instance_name_
                              << "set_multicast_option: failed to configure reuse address, "
                              << _error.message();
                multicast_socket_.reset();
                return;
            }
            ...
            if (_error) {
                VSOMEIP_ERROR << instance_name_ << "set_multicast_option: failed to bind, "
                              << _error.message();
                multicast_socket_.reset();
                return;
            }
            ...
            std::ignore = multicast_socket_->set_option(
                    boost::asio::socket_base::receive_buffer_size(its_udp_recv_buffer_size),
                    _error);
            if (_error) {
                VSOMEIP_ERROR << instance_name_
                              << "set_multicast_option: failed to configure received buffer size, "
                              << _error.message();
                // Non-fatal error
                _error.clear();
            }
            ...
            if (setsockopt(multicast_socket_->native_handle(), SOL_SOCKET, SO_RCVTIMEO, &timeout,
                ...
                == -1) {
                VSOMEIP_ERROR << instance_name_
                              << "set_multicast_option: failed to configure SO_RCVTIMEO, "
                              << _error.message();
                // Non-fatal error
                _error.clear();
            }
            ...
            if (setsockopt(multicast_socket_->native_handle(), SOL_SOCKET, SO_SNDTIMEO, &timeout,
                ...
                == -1) {
                VSOMEIP_ERROR << instance_name_
                              << "set_multicast_option: failed to configure SO_SNDTIMEO, "
                              << _error.message();
                // Non-fatal error
                _error.clear();
            }
            ...
            if (_error) {
                VSOMEIP_ERROR << instance_name_
                              << "set_multicast_option: failed to get received buffer size, "
                              << _error.message();
                multicast_socket_.reset();
                return;
            }
            ...
            // (requires CAP_NET_ADMIN to be successful)
            if (its_option.value() < 0 || its_option.value() < its_udp_recv_buffer_size) {
                _error.assign(setsockopt(multicast_socket_->native_handle(), SOL_SOCKET,
                                         SO_RCVBUFFORCE, &its_udp_recv_buffer_size,
                                         sizeof(its_udp_recv_buffer_size)),
                              boost::system::generic_category());
                if (_error) {
                    VSOMEIP_INFO << instance_name_
                                 << "set_multicast_option: failed to force received buffer size, "
                                 << _error.message();
                    // Non-fatal error
                    _error.clear();
                }
            ...
            VSOMEIP_INFO << instance_name_
                         << "set_multicast_option: start multicast data handler, lifecycle_idx="
                         << lifecycle_idx_.load();
            receive_multicast_unlocked();
        }
        ...
        if (!_error) {
            ...
            VSOMEIP_INFO << instance_name_ << "set_multicast_option: join successful";
        } else {
            VSOMEIP_ERROR << instance_name_ << "set_multicast_option: join failure, "
                          << _error.message() << ", could happen during restart operation";
        ...
        if (multicast_socket_ && multicast_socket_->is_open()) {
            ...
            if (_error) {
                VSOMEIP_ERROR << instance_name_ << "set_multicast_option: leave failure, "
                              << _error.message();
            } else {
                VSOMEIP_INFO << instance_name_ << "set_multicast_option: leave successful";
            }

            if (joined_.empty()) {
                VSOMEIP_INFO << instance_name_ << "set_multicast_option: stop multicast";
                multicast_socket_.reset();
            }
        }
    }
