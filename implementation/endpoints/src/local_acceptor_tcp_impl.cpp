...
void local_acceptor_tcp_impl::async_accept(connection_handler _handler) {
    ...
    auto tmp_remote_ep = std::make_shared<boost::asio::ip::tcp::endpoint>();
    acceptor_->async_accept(*tmp_socket, *tmp_remote_ep,
...
void local_acceptor_tcp_impl::accept_cbk(boost::system::error_code const& _ec, connection_handler _handler,
                                         std::shared_ptr<tcp_socket> _socket, std::shared_ptr<boost::asio::ip::tcp::endpoint> _remote_ep) {
    if (_ec) {
    ...
    auto socket =
            std::make_shared<local_socket_tcp_impl>(io_, std::move(_socket), local_ep_, std::move(*_remote_ep), socket_role_e::SERVER);
