...
std::shared_ptr<endpoint> endpoint_manager_base::create_local_server(const std::shared_ptr<routing_host>& _routing_host) {
        ...
        try {
            ...
            if (its_tmp) {
                ...
                if (!its_error) {
                    ...
                    VSOMEIP_ERROR << "Local UDS server endpoint initialization failed. Client " << std::hex << std::setfill('0')
                                  << std::setw(4) << its_client << " Path: " << its_path.str() << " Reason: " << its_error.message();
                }
            } else {
                VSOMEIP_ERROR << "Local UDS server endpoint creation failed. Client " << std::hex << std::setfill('0') << std::setw(4)
                              << its_client << " Path: " << its_path.str() << " Reason: out_of_memory";
            }
...
#else
    {
        ...
        try {
            ...
            auto its_tmp{std::make_shared<local_tcp_server_endpoint_impl>(shared_from_this(), _routing_host, io_, configuration_, false)};
            if (its_tmp) {
                while (get_local_server_port(its_port, its_used_ports) && !its_local_server) {
                    ...
                    its_tmp->init(its_local_endpoint, its_error);
                    if (!its_error) {
                        VSOMEIP_INFO << __func__ << ": Listening @ " << its_address.to_string() << ":" << std::dec << its_port;

                        if (rm_->is_routing_manager())
                            local_port_ = port_t(configuration_->get_routing_host_port() + 1);
                        else
                            local_port_ = port_t(its_port + 1);
                        VSOMEIP_INFO << __func__ << ": Connecting to other clients from " << its_address.to_string() << ":" << std::dec
                                     << local_port_;
...
void endpoint_manager_base::on_error(const byte_t* _data, length_t _length, endpoint* const _receiver,
...
std::map<client_t, std::shared_ptr<endpoint>> endpoint_manager_base::get_local_endpoints() const {
    std::lock_guard<std::mutex> its_lock(local_endpoint_mutex_);
...
std::shared_ptr<endpoint> endpoint_manager_base::create_local_unlocked(client_t _client) {
    ...
    if (is_local_routing_) {
        its_endpoint = std::make_shared<local_uds_client_endpoint_impl>(shared_from_this(), rm_->shared_from_this(),
                                                                        ...
                                                                        configuration_);
        VSOMEIP_INFO << "Client [" << std::hex << rm_->get_client() << "] is connecting to [" << std::hex << _client << "] at "
        ...
        if (is_guest) {
            try {
                ...
                its_endpoint = std::make_shared<local_tcp_client_endpoint_impl>(
                        shared_from_this(), rm_->shared_from_this(), boost::asio::ip::tcp::endpoint(its_local_address, local_port_),
                        boost::asio::ip::tcp::endpoint(its_remote_address, its_remote_port), io_, configuration_);

                VSOMEIP_INFO << "Client [" << std::hex << std::setfill('0') << std::setw(4) << rm_->get_client() << "] @ "
...
instance_t endpoint_manager_base::find_instance(service_t _service, endpoint* const _endpoint) const {
