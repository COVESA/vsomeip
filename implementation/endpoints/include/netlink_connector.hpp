...
public:
    nl_protocol() { proto = 0; }
    nl_protocol(int _proto) { proto = _proto; }
...
class netlink_connector : public abstract_netlink_connector, public std::enable_shared_from_this<netlink_connector> {
...
public:
    netlink_connector(boost::asio::io_context& _io, const boost::asio::ip::address& _address,
                      const boost::asio::ip::address& _multicast_address, bool _is_requiring_link = true) :
        net_if_index_for_address_(0), handler_(nullptr), socket_(_io),
        recv_buffer_(_multicast_address.is_v4() || _multicast_address.is_v6() ? multicast_buffer_size : default_buffer_size, 0),
        address_(_address), multicast_address_(_multicast_address), is_requiring_link_(_is_requiring_link) { }
    ~netlink_connector() { }
