...
eventgroupentry_impl::eventgroupentry_impl() : reserved_(0) {
    eventgroup_ = 0xFFFF;
    counter_ = 0;
}

eventgroupentry_impl::eventgroupentry_impl(const eventgroupentry_impl& _entry) : entry_impl(_entry), reserved_(0) {
...
bool eventgroupentry_impl::serialize(vsomeip_v3::serializer* _to) const {
    ...
    is_successful = is_successful && _to->serialize(static_cast<uint16_t>(eventgroup_));
...
bool eventgroupentry_impl::matches(const eventgroupentry_impl& _other, const message_impl::options_t& _options) const {
    if (service_ == _other.service_ && instance_ == _other.instance_ && eventgroup_ == _other.eventgroup_
        && major_version_ == _other.major_version_ && counter_ == _other.counter_) {

        // Check, whether options are identical
        if (index1_ == _other.index1_ && index2_ == _other.index2_ && num_options_[0] == _other.num_options_[0]
        ...
        if (num_options_[0] + num_options_[1] != _other.num_options_[0] + _other.num_options_[1]
        ...
        for (const auto option_run : {0, 1}) {
            for (const auto option_index : options_[option_run]) {
                if (its_options_size > option_index) {
                    ...
                    case option_type_e::IP4_ENDPOINT:
                        its_options_current.push_back(std::static_pointer_cast<ipv4_option_impl>(_options[option_index]));
                        break;
                    case option_type_e::IP6_ENDPOINT:
                        its_options_current.push_back(std::static_pointer_cast<ipv6_option_impl>(_options[option_index]));
                        break;
                    default:
                        break;
                    }
            ...
            for (const auto option_index : _other.options_[option_run]) {
                if (its_options_size > option_index) {
                    switch (_options[option_index]->get_type()) {
                    case option_type_e::IP4_ENDPOINT:
                        its_options_other.push_back(std::static_pointer_cast<ipv4_option_impl>(_options[option_index]));
                        break;
                    case option_type_e::IP6_ENDPOINT:
                        its_options_other.push_back(std::static_pointer_cast<ipv6_option_impl>(_options[option_index]));
                        break;
                    default:
                        break;
                    }
...
void eventgroupentry_impl::add_target(const std::shared_ptr<endpoint_definition>& _target) {
...
std::shared_ptr<endpoint_definition> eventgroupentry_impl::get_target(bool _reliable) const {
    return _reliable ? target_reliable_ : target_unreliable_;
}

std::shared_ptr<selective_option_impl> eventgroupentry_impl::get_selective_option() const {
