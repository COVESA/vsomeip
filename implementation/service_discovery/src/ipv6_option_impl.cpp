...
ipv6_option_impl::ipv6_option_impl() : address_({0}) {
    length_ = (1 + 16 + 1 + 1 + 2);
}

ipv6_option_impl::ipv6_option_impl(const boost::asio::ip::address& _address, const uint16_t _port, const bool _is_reliable) :
    ip_option_impl(_port, _is_reliable), address_(_address.to_v6().to_bytes()) {
    type_ = (_address.is_multicast() ? option_type_e::IP6_MULTICAST : option_type_e::IP6_ENDPOINT);
    length_ = (1 + 16 + 1 + 1 + 2);
}

ipv6_option_impl::~ipv6_option_impl() { }

bool ipv6_option_impl::equals(const option_impl& _other) const {
    bool is_equal(ip_option_impl::equals(_other));

    if (is_equal) {
        const ipv6_option_impl& its_other = dynamic_cast<const ipv6_option_impl&>(_other);
...
void ipv6_option_impl::set_address(const ipv6_address_t& _address) {
    ...
    type_ = (its_address.is_multicast() ? option_type_e::IP6_MULTICAST : option_type_e::IP6_ENDPOINT);
}
...
bool ipv6_option_impl::deserialize(vsomeip_v3::deserializer* _from) {
    bool is_successful = option_impl::deserialize(_from) && length_ == VSOMEIP_SD_IPV6_OPTION_LENGTH;
