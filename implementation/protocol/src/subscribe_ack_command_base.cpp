...
subscribe_ack_command_base::subscribe_ack_command_base(id_e _id) :
    command(_id), service_(ANY_SERVICE), instance_(ANY_INSTANCE), eventgroup_(0), subscriber_(0), event_(ANY_EVENT), pending_id_(0) { }
...
void subscribe_ack_command_base::serialize(std::vector<byte_t>& _buffer) const {

    size_t its_size(COMMAND_HEADER_SIZE + sizeof(service_) + sizeof(instance_) + sizeof(eventgroup_) + sizeof(subscriber_) + sizeof(event_)
                    + sizeof(pending_id_));
