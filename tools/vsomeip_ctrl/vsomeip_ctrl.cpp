...
public:
    vsomeip_sender(bool _use_tcp, const std::vector<vsomeip::byte_t>& _user_message, vsomeip::instance_t _instance) :
        use_tcp_(_use_tcp), user_message_(_user_message), instance_(_instance),
        app_(vsomeip::runtime::get()->create_application("vsomeip_ctrl")), wait_service_available_(true), service_id_(0x0), method_id_(0x0),
        length_(0), client_id_(0x0), interface_version_(0x0), message_type_(vsomeip::message_type_e::MT_UNKNOWN),
        return_code_(vsomeip::return_code_e::E_UNKNOWN), wait_for_answer_(true) {
        ...
        app_->register_state_handler(std::bind(&vsomeip_sender::on_state, this, std::placeholders::_1));
        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
                                       std::bind(&vsomeip_sender::on_message, this, std::placeholders::_1));
        app_->register_availability_handler(
                service_id_, instance_,
                std::bind(&vsomeip_sender::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        app_->start();
    };
    ...
    ~vsomeip_sender() { send_thread_.join(); }
    ...
    void on_message(const std::shared_ptr<vsomeip::message>& _response) {
        ...
        VSOMEIP_INFO << std::hex << std::setfill('0') << std::setw(4) << _response->get_service() << std::setw(4) << _response->get_method()
...
private:
    bool validate_message() {
        ...
        if (service_id_ == vsomeip::sd::service && method_id_ == vsomeip::sd::method) {
        ...
        if (user_message_.size() != length_ + 8) {
            VSOMEIP_ERROR << "Provided length 0x" << std::setw(8) << std::hex << std::setfill('0') << length_
                          << " doesn't match message size.";
            VSOMEIP_ERROR << "Assuming the same payload the length field should"
                             " be set to 0x"
                          << std::setw(8) << std::setfill('0') << std::hex << user_message_.size() - 8 << " , exiting.";
            stop(EXIT_FAILURE);
        }

        if (use_tcp_ && user_message_.size() > VSOMEIP_MAX_TCP_MESSAGE_SIZE) {
            VSOMEIP_WARNING << "Max allowed message size for TCP is " << std::dec << VSOMEIP_MAX_TCP_MESSAGE_SIZE
                            << ". Provided message size is: " << user_message_.size();
        }
        if (!use_tcp_ && user_message_.size() > VSOMEIP_MAX_UDP_MESSAGE_SIZE) {
            VSOMEIP_WARNING << "Max allowed message size for UDP is " << std::dec << VSOMEIP_MAX_UDP_MESSAGE_SIZE
                            << ". Provided message size is: " << user_message_.size();
        }
        return true;
    }
...
static void print_help(char* binary_name) {
    ...
    std::cout << "Available options:\n"
                 ...
                 "as the target service, if the system is not using routingmanagerd"
              << std::endl;
}

int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "To few arguments, please see the help with : " << argv[0] << " --help" << std::endl;
        exit(EXIT_FAILURE);
    }
        ...
        if (arg == "--tcp" || arg == "-t") {
            ...
            for (unsigned int i = 0; i < instance_str.length(); i += 2) {
                try {
                    ...
                    std::cerr << e.what() << ": Couldn't convert '" << instance_str.substr(i, 2) << "' to hex, exiting: " << std::endl;
                    exit(EXIT_FAILURE);
                }
            }
