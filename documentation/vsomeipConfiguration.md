...
- [vSomeIP Configurations](#vsomeip-configurations)
  - [Index](#index)
  - [Logging](#logging)
  - [Routing](#routing)
    - [Server side configuration](#server-side-configuration)
    - [Client side configuration](#client-side-configuration)
    - [Server side configuration](#server-side-configuration-1)
    - [Client side configuration](#client-side-configuration-1)
...
[
    {
        ...
        "threads" : "4",
        "io_thread_nice" : "-5",
        "status_log_interval": "1000",
        "version_log_interval": "5000",
        "plugins" :
        [
            {
...
- **service-discovery** - Contains settings related to the Service Discovery of the host application.
    ...
    - **stop_offer_watchdog_time** - (optional) - Configures the timeout for the watchdog that tracks if all boardnet services being offered have a corresponding stop offer on suspend. Enabled by default with a value of `2000` ms, if set to zero, it disables the watchdog. If the timeout is reached,
    the occurrence is logged.
    - **offer_watchdog_time** - (optional) - Configures the timeout for the watchdog that tracks if any boardnet services are being offered after a suspend. Enabled by default with a value of `4000` ms, if set to zero, it disables the watchdog. If the timeout is reached, vsomeip aborts.
