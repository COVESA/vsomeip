...
bool protection_option_impl::operator ==(const option& _other) const {
	...
	const protection_option_impl& other
		= reinterpret_cast<const protection_option_impl&>(_other);

	return (crc_ == other.crc_ && counter_ == other.counter_);
}
...
bool protection_option_impl::deserialize(vsomeip::deserializer *_from) {
	...
	is_successful = is_successful && _from->deserialize(tmp_counter);
	counter_ = static_cast<alive_counter>(tmp_counter);

	uint32_t tmp_crc = 0;
	is_successful = is_successful && _from->deserialize(tmp_crc);
	crc_ = static_cast<alive_counter>(tmp_crc);
