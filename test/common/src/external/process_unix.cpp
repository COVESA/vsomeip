#ifdef __linux__
#include <common/external/process.hpp>
#include <algorithm>
#include <bitset>
#include <cstdlib>
#include <errno.h>
...
Process::Process(const std::function<void()>& function, std::function<void(const char*, size_t)> read_stdout,
                 std::function<void(const char*, size_t)> read_stderr, bool open_stdin, const Config& config) :
...
Process::id_type Process::open(const std::function<void()>& function) noexcept {
    ...
    if (pid < 0) {
        ...
        if (!config.inherit_file_descriptors) {
            // Optimization on some systems: using 8 * 1024 (Debian's default _SC_OPEN_MAX) as
            // fd_max limit
...
Process::id_type Process::open(const std::string& command, const std::string& path, const environment_type* environment) noexcept {
    return open([this, &command, &path, &environment] {
        ...
        if (!environment) {
            ...
            if (config.flatpak_spawn_host)
                // break out of sandbox, execute on host
                execle("/usr/bin/flatpak-spawn", "/usr/bin/flatpak-spawn", "--host", "/bin/sh", "-c", command_c_str, nullptr,
                       env_ptrs.data());
            else
...
void Process::kill(bool force) noexcept {
    ...
    if (data.id > 0 && !closed) {
        if (force) {
            ...
            ::kill(data.id,
                   SIGTERM); // Based on comment in
                             // https://gitlab.com/eidheim/tiny-process-library/-/merge_requests/29#note_1146144166
        } else {
            ::kill(-data.id, SIGINT);
            ::kill(data.id, SIGINT);
        }
