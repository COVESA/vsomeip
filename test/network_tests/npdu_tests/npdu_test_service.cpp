...
void npdu_test_service::init() {
    ...
    app_->register_message_handler(service_id_, instance_id_, npdu_test::NPDU_SERVICE_SHUTDOWNMETHOD_ID,
                                   std::bind(&npdu_test_service::on_message_shutdown, this, std::placeholders::_1));

    app_->register_state_handler(std::bind(&npdu_test_service::on_state, this, std::placeholders::_1));
}
...
void npdu_test_service::register_message_handler() {
    app_->register_message_handler(service_id_, instance_id_, method_ids_[method_idx],
                                   std::bind(&npdu_test_service::on_message<method_idx>, this, std::placeholders::_1));
}
...
void npdu_test_service::check_times() {
    ...
    std::chrono::nanoseconds time_since_last_message =
            std::chrono::duration_cast<std::chrono::nanoseconds>(now - timepoint_last_received_message_[method_idx]);
    ...
    if (time_since_last_message < debounce_times_[method_idx]) {
        const auto time_undershot =
                std::chrono::duration_cast<std::chrono::microseconds>(debounce_times_[method_idx] - time_since_last_message);
...
void npdu_test_service::on_message(const std::shared_ptr<vsomeip::message>& _request) {
    ...
    VSOMEIP_DEBUG << "0x" << std::hex << std::setfill('0') << std::setw(4) << method_ids_[method_idx] << " payload size: " << std::dec
...
void npdu_test_service::on_message_shutdown(const std::shared_ptr<vsomeip::message>& _request) {
    (void)_request;
