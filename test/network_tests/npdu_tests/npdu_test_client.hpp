...
private:
    ...
    template<int service_idx>
    void register_message_handler_for_all_service_methods();
    template<int service_idx, int method_idx>
...
private:
    ...
    std::array<std::array<std::unique_lock<std::mutex>, npdu_test::method_ids[0].size()>, npdu_test::service_ids.size()>
