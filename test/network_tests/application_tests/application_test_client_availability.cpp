...
public:
    application_test_client_availability(struct application_test::service_info _service_info) :
        service_info_(_service_info), app_(vsomeip::runtime::get()->create_application("client")), availability_handler_called_{},
        wait_until_registered_(true), all_availability_handlers_called_(false),
        run_thread_(std::bind(&application_test_client_availability::run, this)) {
        if (!app_->init()) {
            ADD_FAILURE() << "Couldn't initialize application";
            return;
        }
        app_->register_state_handler(std::bind(&application_test_client_availability::on_state, this, std::placeholders::_1));
        ...
        for (std::uint32_t i = 0; i < 16; i++) {
            ...
            app_->register_availability_handler(its_service, its_instance,
    ...
    void on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available, std::uint32_t _handler_index) {
        ...
        if (service_info_.service_id == _service && service_info_.instance_id == _instance) {
            std::scoped_lock its_lock(availability_handler_called_mutex_);
    ...
    bool all_availability_handlers_called() const { return all_availability_handlers_called_; }
