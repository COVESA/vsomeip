...
public:
    second_address_test_service(struct second_address_test::service_info _service_info) :
        ...
        app_->register_state_handler(std::bind(&second_address_test_service::on_state, this, std::placeholders::_1));

        app_->register_message_handler(service_info_.service_id, service_info_.instance_id, service_info_.request_method_id,
                                       std::bind(&second_address_test_service::on_message, this, std::placeholders::_1));

        app_->register_message_handler(service_info_.service_id, service_info_.instance_id, service_info_.notify_method_id,
                                       std::bind(&second_address_test_service::on_notify, this, std::placeholders::_1));

        app_->register_message_handler(service_info_.service_id, service_info_.instance_id, service_info_.shutdown_method_id,
                                       std::bind(&second_address_test_service::on_shutdown_method_called, this, std::placeholders::_1));

        app_->register_subscription_handler(service_info_.service_id, service_info_.instance_id, service_info_.eventgroup_id,
                                            std::bind(&second_address_test_service::subscription_handler, this, std::placeholders::_1,
                                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));

        app_->register_subscription_handler(service_info_.service_id, service_info_.instance_id, service_info_.selective_eventgroup_id,
                                            std::bind(&second_address_test_service::selective_subscription_handler, this,
...
private:
    ...
    void offer() {
        ...
        app_->offer_event(service_info_.service_id, service_info_.instance_id, service_info_.selective_event_id, its_eventgroups,
    ...
    void notify() {
        ...
        for (; i < 2 * notifications_to_send_; i++) {
            its_payload->set_data(std::vector<vsomeip::byte_t>(i + 1, 0x55));
            app_->notify_one(service_info_.service_id, service_info_.instance_id, service_info_.selective_event_id, its_payload,
                             client_id_);
        }
    }
