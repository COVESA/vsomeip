...
public:
    offer_test_big_sd_msg_service(struct offer_test::service_info _service_info) :
        ...
        app_(vsomeip::runtime::get()->create_application("offer_test_big_sd_msg_service")), wait_until_registered_(true),
        wait_until_client_subscribed_to_all_services_(true), shutdown_method_called_(false),
        offer_thread_(std::bind(&offer_test_big_sd_msg_service::run, this)) {
        if (!app_->init()) {
            ADD_FAILURE() << "Couldn't initialize application";
            return;
        }
        app_->register_state_handler(std::bind(&offer_test_big_sd_msg_service::on_state, this, std::placeholders::_1));
        ...
        for (std::uint16_t s = 1; s <= offer_test::big_msg_number_services; s++) {
            app_->offer_event(s, 0x1, offer_test::big_msg_event_id, its_eventgroups, vsomeip::event_type_e::ET_EVENT,
            ...
            app_->register_subscription_handler(s, 0x1, offer_test::big_msg_eventgroup_id,
                                                std::bind(&offer_test_big_sd_msg_service::on_subscription, this, std::placeholders::_1,
                                                          std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, s));
            subscriptions_[s] = 0;
        }
    ...
    ~offer_test_big_sd_msg_service() { offer_thread_.join(); }
    ...
    bool on_subscription(vsomeip::client_t _client, std::uint32_t _uid, std::uint32_t _gid, bool _subscribed, vsomeip::service_t _service) {
        (void)_client;
        (void)_uid;
        (void)_gid;
        if (_subscribed) {
