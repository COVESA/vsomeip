...
public:
    offer_test_client(struct offer_test::service_info _service_info, operation_mode_e _mode) :
        ...
        app_->register_state_handler(std::bind(&offer_test_client::on_state, this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
                                       std::bind(&offer_test_client::on_message, this, std::placeholders::_1));
        ...
        app_->register_availability_handler(
                ...
                std::bind(&offer_test_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        ...
        if (operation_mode_ == operation_mode_e::SUBSCRIBE) {
            ...
            app_->request_event(service_info_.service_id, service_info_.instance_id, service_info_.event_id, its_eventgroups,
                                vsomeip::event_type_e::ET_EVENT, vsomeip::reliability_type_e::RT_BOTH);

            app_->subscribe(service_info_.service_id, service_info_.instance_id, service_info_.eventgroup_id, vsomeip::DEFAULT_MAJOR);
        }
    ...
    void on_notification(const std::shared_ptr<vsomeip::message>& _message) {
        ...
        VSOMEIP_DEBUG << "Received a notification with Client/Session [" << std::hex << std::setfill('0') << std::setw(4)
                      ...
                      << std::setw(4) << _message->get_service() << "/" << std::setw(4) << _message->get_method() << "] got:" << std::dec
                      << counter;
    ...
    void on_response(const std::shared_ptr<vsomeip::message>& _message) {
        ...
        EXPECT_LT(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last_received_response_).count(),
                  (std::chrono::milliseconds(VSOMEIP_DEFAULT_PING_TIMEOUT) + std::chrono::milliseconds(10000)).count());
        last_received_response_ = std::chrono::steady_clock::now();
        std::cout << ".";
        std::cout.flush();
    }
