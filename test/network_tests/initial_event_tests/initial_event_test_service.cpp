...
public:
    initial_event_test_service(struct initial_event_test::service_info _service_info, std::uint16_t _events_to_offer,
        ...
        app_->register_state_handler(std::bind(&initial_event_test_service::on_state, this, std::placeholders::_1));
...
TEST(someip_initial_event_test, set_field_once) {
    if (use_same_service_id) {
        initial_event_test_service its_sample(initial_event_test::service_infos_same_service_id[service_number], offer_multiple_events,
                                              reliability_type);
    } else {
        initial_event_test_service its_sample(initial_event_test::service_infos[service_number], offer_multiple_events, reliability_type);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        std::cerr << " - SAME_SERVICE_ID flag. If set the test is run w/ multiple instances of the "
                  ...
                  << std::endl;
        std::cerr << " - MULTIPLE_EVENTS flag. If set the test will offer to multiple events in "
                     "the eventgroup, default false"
                  << std::endl;
        return 1;
    }
