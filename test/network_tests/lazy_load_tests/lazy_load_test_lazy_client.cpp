...
lazy_load_lazy_client::lazy_load_lazy_client() :
    app_(vsomeip::runtime::get()->create_application()), current_service_availability_status_(false), received_responses_(0),
...
bool lazy_load_lazy_client::init() {
    ...
    app_->register_state_handler(std::bind(&lazy_load_lazy_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip_test::TEST_SERVICE_INSTANCE_ID, vsomeip::ANY_METHOD,
                                   std::bind(&lazy_load_lazy_client::on_message, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip::ANY_SERVICE, TEST_INSTANCE_LAZY, vsomeip::ANY_METHOD,
                                   std::bind(&lazy_load_lazy_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(
            vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
            std::bind(&lazy_load_lazy_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    app_->register_availability_handler(
            SERVICE_TO_BE_REFUSED, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
            std::bind(&lazy_load_lazy_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    app_->register_availability_handler(
            vsomeip_test::TEST_SERVICE_SERVICE_ID, TEST_INSTANCE_LAZY,
            std::bind(&lazy_load_lazy_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void lazy_load_lazy_client::on_state(vsomeip::state_type_e _state) {
    ...
    if (_state == vsomeip::state_type_e::ST_REGISTERED) {
        ...
        app_->request_service(SERVICE_TO_BE_REFUSED, vsomeip_test::TEST_SERVICE_INSTANCE_ID, false);

        app_->request_service(vsomeip_test::TEST_SERVICE_SERVICE_ID, TEST_INSTANCE_LAZY, false);
        ...
        app_->request_event(vsomeip_test::TEST_SERVICE_SERVICE_ID, TEST_INSTANCE_LAZY, static_cast<vsomeip::event_t>(EVENT_TO_REFUSE_LAZY),
                            its_eventgroups, vsomeip::event_type_e::ET_FIELD, vsomeip::reliability_type_e::RT_UNRELIABLE);
        app_->request_event(vsomeip_test::TEST_SERVICE_SERVICE_ID, TEST_INSTANCE_LAZY, static_cast<vsomeip::event_t>(EVENT_TO_ACCEPT_LAZY),
                            its_eventgroups, vsomeip::event_type_e::ET_FIELD, vsomeip::reliability_type_e::RT_UNRELIABLE);

        app_->subscribe(vsomeip_test::TEST_SERVICE_SERVICE_ID, TEST_INSTANCE_LAZY, EVENT_GROUP, vsomeip::DEFAULT_MAJOR,
...
void lazy_load_lazy_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_service_available) {
    ...
    VSOMEIP_INFO << std::hex << "Client 0x" << app_->get_client() << " : Service [" << std::hex << std::setfill('0') << std::setw(4)
                 << _service << '.' << _instance << "] is " << (_is_service_available ? "available." : "NOT available.");

    // Check that only the allowed service is available
    if (_is_service_available) {
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_SERVICE_ID, _service) << "Unexpected Service ID on_available";
        EXPECT_EQ(TEST_INSTANCE_LAZY, _instance) << "Unexpected Instance ID on_available";
    }

    if (vsomeip_test::TEST_SERVICE_SERVICE_ID == _service && TEST_INSTANCE_LAZY == _instance) {
...
void lazy_load_lazy_client::on_message(const std::shared_ptr<vsomeip::message>& _response) {
    ...
    if (_response->get_message_type() == vsomeip::message_type_e::MT_RESPONSE) {
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_SERVICE_ID, _response->get_service()) << "Unexpected Service ID on_message response";
        EXPECT_EQ(TEST_INSTANCE_LAZY, _response->get_instance()) << "Unexpected Instance ID on_message response";
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_METHOD_ID, _response->get_method()) << "Unexpected Method ID on_message response";
        ...
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_SERVICE_ID, _response->get_service()) << "Unexpected Service ID on_message notification";
        EXPECT_EQ(TEST_INSTANCE_LAZY, _response->get_instance()) << "Unexpected Instance ID on_message notification";
        EXPECT_EQ(EVENT_TO_ACCEPT_LAZY, _response->get_method()) << "Unexpected Method ID on_message notification";
...
void lazy_load_lazy_client::run() {
    ...
    EXPECT_EQ(received_allowed_events_, static_cast<std::uint32_t>(EXPECTED_EVENTS)) << "Unexpected received_allowed_events_ run";
