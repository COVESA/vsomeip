...
bool lazy_load_test_client::init() {
    ...
    app_->register_state_handler(std::bind(&lazy_load_test_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip_test::TEST_SERVICE_INSTANCE_ID, vsomeip::ANY_METHOD,
                                   std::bind(&lazy_load_test_client::on_message, this, std::placeholders::_1));
    ...
    app_->register_availability_handler(
            ...
            std::bind(&lazy_load_test_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void lazy_load_test_client::on_state(vsomeip::state_type_e _state) {
    ...
    if (_state == vsomeip::state_type_e::ST_REGISTERED) {
        ...
        app_->request_event(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                            static_cast<vsomeip::event_t>(EVENT_TO_ACCEPT), its_eventgroups, vsomeip::event_type_e::ET_FIELD,
                            vsomeip::reliability_type_e::RT_UNRELIABLE);
        app_->request_event(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                            static_cast<vsomeip::event_t>(EVENT_TO_REFUSE), its_eventgroups, vsomeip::event_type_e::ET_FIELD,
                            vsomeip::reliability_type_e::RT_UNRELIABLE);

        app_->subscribe(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID, EVENT_GROUP, vsomeip::DEFAULT_MAJOR,
...
void lazy_load_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_service_available) {
    ...
    if (_is_service_available) {
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_SERVICE_ID, _service) << "Unexpected Service ID on_availability";
...
void lazy_load_test_client::on_message(const std::shared_ptr<vsomeip::message>& _response) {
    ...
    if (_response->get_message_type() == vsomeip::message_type_e::MT_RESPONSE) {
        ...
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_INSTANCE_ID, _response->get_instance()) << "Unexpected Instance ID on_message response";
        ...
        EXPECT_EQ(vsomeip_test::TEST_SERVICE_INSTANCE_ID, _response->get_instance()) << "Unexpected Instance ID on_message notification";
        EXPECT_EQ(EVENT_TO_ACCEPT, _response->get_method()) << "Unexpected Method ID on_message notification";
