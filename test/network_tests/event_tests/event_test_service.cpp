...
public:
    event_test_service(struct event_test::service_info _service_info, bool _use_tcp) :
        ...
        app_->register_state_handler(std::bind(&event_test_service::on_state, this, std::placeholders::_1));
        ...
        app_->register_message_handler(service_info_.service_id, service_info_.instance_id, service_info_.shutdown_method_id,
                                       std::bind(&event_test_service::on_shutdown_method_called, this, std::placeholders::_1));
        app_->register_message_handler(service_info_.service_id, service_info_.instance_id, service_info_.notify_method_id,
                                       std::bind(&event_test_service::on_message, this, std::placeholders::_1));
        app_->register_subscription_handler(service_info_.service_id, service_info_.instance_id, service_info_.eventgroup_id,
                                            std::bind(&event_test_service::subscription_handler, this, std::placeholders::_1,
                                                      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));

        app_->start();
    }

    ~event_test_service() { offer_thread_.join(); }
    ...
    void notify() {
        ...
        for (std::uint32_t i = 0; i < notifications_to_send_; i++) {
            ...
            app_->notify(service_info_.service_id, service_info_.instance_id, service_info_.event_id, its_payload, false);
        }
    }
