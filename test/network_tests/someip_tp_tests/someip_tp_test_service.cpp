...
public:
    ...
    void start() {
        ...
        app_->register_state_handler(std::bind(&someip_tp_test_service::on_state, this, std::placeholders::_1));
        ...
        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, service_info_.shutdown_method_id,
                                       std::bind(&someip_tp_test_service::on_shutdown_method_called, this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, service_info_.notify_method_id,
                                       std::bind(&someip_tp_test_service::on_notify_method_called, this, std::placeholders::_1));
        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, service_info_.method_id,
                                       std::bind(&someip_tp_test_service::on_message, this, std::placeholders::_1));

        app_->register_async_subscription_handler(service_info_.service_id, 0x1, service_info_.eventgroup_id,
                                                  std::bind(&someip_tp_test_service::subscription_handler_async, this,
                                                            std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                                                            std::placeholders::_4, std::placeholders::_5));

        // request remote service
        app_->request_service(someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id);
        its_eventgroups.clear();
        its_eventgroups.insert(someip_tp_test::service_slave.eventgroup_id);
        app_->request_event(someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id,
                            someip_tp_test::service_slave.event_id, its_eventgroups, vsomeip::event_type_e::ET_EVENT,
                            vsomeip::reliability_type_e::RT_UNRELIABLE);
        app_->register_message_handler(someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id,
                                       someip_tp_test::service_slave.event_id,
                                       std::bind(&someip_tp_test_service::on_notification, this, std::placeholders::_1));
        app_->subscribe(someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id,
                        someip_tp_test::service_slave.eventgroup_id, 0x0, someip_tp_test::service_slave.event_id);
        app_->register_availability_handler(someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id,
                                            std::bind(&someip_tp_test_service::on_availability, this, std::placeholders::_1,
    ...
    void on_message(const std::shared_ptr<vsomeip::message>& _message) {
        VSOMEIP_INFO << "Received a message with Client/Session [" << std::hex << std::setfill('0') << std::setw(4)
                     << _message->get_client() << "/" << std::setw(4) << _message->get_session() << "] size: " << std::dec
                     << _message->get_payload()->get_length();
    ...
    void on_notification(const std::shared_ptr<vsomeip::message>& _message) {
        VSOMEIP_INFO << "Received a notification with Client/Session [" << std::hex << std::setfill('0') << std::setw(4)
                     << _message->get_client() << "/" << std::setw(4) << _message->get_session() << "] size: " << std::dec
                     << _message->get_payload()->get_length();
        ...
        std::vector<vsomeip::byte_t> its_cmp_data = generate_payload(
                someip_tp_test::number_of_fragments,
                (testmode_ == someip_tp_test::test_mode_e::OVERLAP || testmode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
    ...
    void on_notify_method_called(const std::shared_ptr<vsomeip::message>& _message) {
        ...
        std::vector<vsomeip::byte_t> its_data =
                generate_payload(someip_tp_test::number_of_fragments,
                                 (testmode_ == someip_tp_test::test_mode_e::OVERLAP) ? someip_tp_test::max_segment_size - 160
                                                                                     : someip_tp_test::max_segment_size);
    ...
    void send_fragmented_request_to_slave() {
        ...
        std::vector<vsomeip::byte_t> its_data = generate_payload(
                someip_tp_test::number_of_fragments,
                (testmode_ == someip_tp_test::test_mode_e::OVERLAP || testmode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                        ? someip_tp_test::max_segment_size - 160
                        : someip_tp_test::max_segment_size);
    ...
    void on_response_from_slave(const std::shared_ptr<vsomeip::message>& _message) {
        VSOMEIP_INFO << "Received a response from the slave with Client/Session [" << std::hex << std::setfill('0') << std::setw(4)
                     << _message->get_client() << "/" << std::setw(4) << _message->get_session() << "] size: " << std::dec
                     << _message->get_payload()->get_length();
        EXPECT_EQ(someip_tp_test::service_slave.service_id, _message->get_service());
        ...
        std::vector<vsomeip::byte_t> its_req_payload(request_send_to_slave_->get_payload()->get_data(),
                                                     request_send_to_slave_->get_payload()->get_data()
        ...
        EXPECT_EQ(0,
                  std::memcmp(static_cast<void*>(&its_req_payload[0]), static_cast<void*>(&its_resp_payload[0]), its_req_payload.size()));
