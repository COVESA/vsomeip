...
local_routing_test_client::local_routing_test_client(bool _use_tcp) :
    app_(vsomeip::runtime::get()->create_application()), request_(vsomeip::runtime::get()->create_request(_use_tcp)), blocked_(false),
    is_available_(false), number_of_messages_to_send_(vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND), number_of_sent_messages_(0),
    number_of_acknowledged_messages_(0), sender_(std::bind(&local_routing_test_client::run, this)) { }
...
bool local_routing_test_client::init() {
    ...
    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip_test::TEST_SERVICE_INSTANCE_ID, vsomeip::ANY_METHOD,
                                   std::bind(&local_routing_test_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                                        std::bind(&local_routing_test_client::on_availability, this, std::placeholders::_1,
...
void local_routing_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
...
void local_routing_test_client::run() {
    ...
    for (uint32_t i = 0; i < number_of_messages_to_send_; i++) {
        ...
        VSOMEIP_INFO << "Client/Session [" << std::hex << std::setfill('0') << std::setw(4) << request_->get_client() << "/" << std::setw(4)
                     << request_->get_session() << "] sent a request to Service [" << std::setw(4) << request_->get_service() << "."
