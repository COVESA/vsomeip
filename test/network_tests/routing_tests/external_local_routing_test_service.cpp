...
external_local_routing_test_service::external_local_routing_test_service(bool _use_static_routing) :
    ...
    number_received_messages_local_(0), number_received_messages_external_(0),
...
bool external_local_routing_test_service::init() {
    ...
    app_->register_message_handler(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                                   ...
                                   std::bind(&external_local_routing_test_service::on_message, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                                   vsomeip_test::TEST_SERVICE_METHOD_ID,
                                   std::bind(&external_local_routing_test_service::on_message, this, std::placeholders::_1));

    app_->register_state_handler(std::bind(&external_local_routing_test_service::on_state, this, std::placeholders::_1));
...
void external_local_routing_test_service::on_message(const std::shared_ptr<vsomeip::message>& _request) {
    ...
    ASSERT_LT(number_received_messages_local_, vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND + 1);
    ASSERT_LT(number_received_messages_external_, vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND + 1);
}
