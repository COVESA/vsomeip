...
void cpu_load_measurer::stop() {
    ...
    cpu_load_pid_ = 100.0 * static_cast<double>(jiffies_passed_pid_stop_ - jiffies_passed_pid_start_)
            / static_cast<double>(jiffies_complete_stop_ - jiffies_complete_start_);
    cpu_load_overall_ = 100.0
            * static_cast<double>((jiffies_complete_stop_ - jiffies_idle_stop_) - (jiffies_complete_start_ - jiffies_idle_start_))
            / static_cast<double>(jiffies_complete_stop_ - jiffies_complete_start_);
    cpu_load_pid_wo_idle_ = 100.0 * static_cast<double>(jiffies_passed_pid_stop_ - jiffies_passed_pid_start_)
            / static_cast<double>((jiffies_complete_stop_ - jiffies_idle_stop_) - (jiffies_complete_start_ - jiffies_idle_start_));
}

void cpu_load_measurer::print_cpu_load() const {
    std::cout << "Used Jiffies complete: " << jiffies_complete_stop_ - jiffies_complete_start_
              << " (worked: " << (jiffies_complete_stop_ - jiffies_idle_stop_) - (jiffies_complete_start_ - jiffies_idle_start_)
              << " idled: " << jiffies_idle_stop_ - jiffies_idle_start_ << ")" << std::endl;
