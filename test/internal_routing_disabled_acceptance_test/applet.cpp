...
void applet::init() {
    ...
    this->async_start = std::async(std::launch::async, &vsomeip_v3::application::start, this->application);

    this->application->register_state_handler([its_me](vsomeip_v3::state_type_e state) {
        auto me = its_me.lock();
        if (me) {
            switch (state) {
