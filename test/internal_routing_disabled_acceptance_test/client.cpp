...
void client::init() {
    ...
    std::weak_ptr<client> its_me = std::dynamic_pointer_cast<client>(shared_from_this());

    this->application->register_message_handler(
            config::SERVICE_ID, config::INSTANCE_ID, vsomeip_v3::ANY_METHOD, [its_me](const std::shared_ptr<vsomeip_v3::message>& message) {
                auto me = its_me.lock();
                if (me) {
                    ...
                    default:
                        VSOMEIP_ERROR << "unhandled message type: " << unsigned(message->get_message_type());
                    }
                }
            });
    ...
    this->application->register_availability_handler(
            ...
            [its_me](vsomeip_v3::service_t service, vsomeip_v3::instance_t instance, bool available) {
                auto me = its_me.lock();
                if (me) {
                    VSOMEIP_INFO << '(' << std::hex << service << ", " << std::hex << instance << ", " << std::boolalpha << available
                                 << ")";
    ...
    this->application->request_event(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID, {config::EVENTGROUP_ID},
                                     vsomeip_v3::event_type_e::ET_FIELD, vsomeip_v3::reliability_type_e::RT_UNRELIABLE);

    this->application->subscribe(config::SERVICE_ID, config::INSTANCE_ID, config::EVENTGROUP_ID);
}

client::~client() {
    this->application->unsubscribe(config::SERVICE_ID, config::INSTANCE_ID, config::EVENTGROUP_ID);

    this->application->release_event(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID);

    this->application->release_service(config::SERVICE_ID, config::INSTANCE_ID);

    this->application->unregister_availability_handler(config::SERVICE_ID, config::INSTANCE_ID);

    this->application->unregister_message_handler(config::SERVICE_ID, config::INSTANCE_ID, vsomeip_v3::ANY_METHOD);
}
...
void client::on_state_registered() {
    this->application->request_service(config::SERVICE_ID, config::INSTANCE_ID);
}
