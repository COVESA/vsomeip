...
void server::init() {
    ...
    std::weak_ptr<server> its_me = std::dynamic_pointer_cast<server>(shared_from_this());

    this->application->register_message_handler(
            config::SERVICE_ID, config::INSTANCE_ID, config::METHOD_ID, [its_me](const std::shared_ptr<vsomeip_v3::message>& message) {
                auto me = its_me.lock();
                if (me) {
                    ...
                    case vsomeip_v3::message_type_e::MT_REQUEST:
                        ...
                        me->counter_method_request++;
                        {
                            ...
                            me->application->notify(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID, payload, true);
    ...
    this->application->offer_event(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID, {config::EVENTGROUP_ID},
                                   vsomeip_v3::event_type_e::ET_FIELD, {}, false, true, nullptr,
...
server::~server() {
    this->application->stop_offer_event(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID);

    this->application->stop_offer_service(config::SERVICE_ID, config::INSTANCE_ID);
}
...
void server::on_state_registered() {
    this->application->offer_service(config::SERVICE_ID, config::INSTANCE_ID);
}
