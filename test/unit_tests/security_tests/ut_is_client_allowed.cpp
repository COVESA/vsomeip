...
TEST(is_client_allowed_test, check_policies_loaded) {
    ...
    // test is_client_allowed_cache_, request with the same credentials and service / instance /
    // method
    EXPECT_TRUE(its_manager->is_client_allowed(&its_sec_client, service_1, instance, method));

    // test is_client_allowed_cache_, request with the same credentials and service but with a
    // different instance or method is_client_allowed return true because it's define ANY_INSTANCE
    // and ANY_METHOD in the policy
