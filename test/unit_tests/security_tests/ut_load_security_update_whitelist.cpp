...
TEST(load_security_update_whitelist, check_uids) {
    ...
    ASSERT_FALSE(security->is_policy_removal_allowed(valid_uid)) << "The whitelist unexpectedly holds a valid uid";

    ASSERT_FALSE(security->is_policy_removal_allowed(invalid_uid)) << "The whitelist unexpectedly holds an invalid uid";
    ...
    // Check that the valid and invalid uids are not present in the whitelist by calling a method
    // that verifies that condition.
    ASSERT_TRUE(security->is_policy_removal_allowed(valid_uid)) << "The whitelist expected to hold a valid uid";
...
TEST(load_security_update_whitelist, check_service_ids) {
    ...
    ASSERT_FALSE(security->is_policy_update_allowed(valid_uid, policy)) << "The whitelist unexpectedly holds a valid service_id";

    // Add an invalid request to the policy
    policy->requests_ += std::make_pair(boost::icl::discrete_interval<vsomeip::service_t>(its_invalid_service, its_invalid_service,
                                                                                          boost::icl::interval_bounds::closed()),
                                        its_instances_methods);

    // Check its presence using the is_policy_update_allowed method.
    ASSERT_FALSE(security->is_policy_update_allowed(valid_uid, policy)) << "The whitelist unexpectedly holds an invalid service_id";
    ...
    // Check its presence using the is_policy_update_allowed method.
    ASSERT_TRUE(security->is_policy_update_allowed(valid_uid, policy)) << "The whitelist expected to hold a valid service_id";
    ...
    // Check its presence using the is_policy_update_allowed method.
    ASSERT_FALSE(security->is_policy_update_allowed(invalid_uid, policy)) << "The whitelist unexpectedly holds an invalid service_id";
}
...
TEST(load_security_update_whitelist, check_whitelist_disabled) {
    ...
    ASSERT_TRUE(security->is_policy_removal_allowed(valid_uid)) << "The whitelist is disabled, a valid uid should be allowed to be removed";
