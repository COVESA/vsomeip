...
TEST(load_policies, any_policies_present) {
    ...
    ASSERT_FALSE(security->remove_security_policy(valid_uid, valid_gid)) << "Trying to remove a policy should not exist, but it exists";
}
...
TEST(load_policies, check_credentials) {
    ...
    ASSERT_TRUE(security->is_audit()) << "Check credentials value should be false when no value is loaded";
    ...
    ASSERT_FALSE(security->is_audit()) << "Check credentials flag should be true when the check_credential value is loaded as"
                                          "true";
    ...
    ASSERT_TRUE(security->is_audit()) << "Check credentials flag should be false when the check_credential value is loaded as"
                                         "false";
}
