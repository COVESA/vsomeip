...
TEST(deserialize_test, deserialize_message) {
    ...
    auto its_deserializer2 = std::make_unique<vsomeip_v3::deserializer>(byte_array2_.data(), byte_array2_.size(), buffer_shrink_threshold);

    auto deserialized_message2 = its_deserializer2->deserialize_message();

    // Deserialize expected to fail.
